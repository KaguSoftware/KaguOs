            ...
            <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Email" htmlFor="new-email">
                    <EmailInput
                        ...
                        name="email"
                        required
                        placeholder="teammate@kagusoftware.com"
                    />
                </Field>
            ...
            {kind === "member" ? (
                ...
                uncontrolled form — parseAccess in actions/admin.ts reads the pair. */}
                        ...
                        <p className="mt-1.5 text-[calc(13px*var(--text-scale,1))] text-faint">
                            Sections are independent — tick exactly what they
                            should see. &ldquo;View only&rdquo; lets them read
                            the section without changing anything in it.
                        </p>
                    </fieldset>
                    <Checkbox
                        name="is_admin"
                        label="Admin (manages users and sprints)"
                    />
                </>
            ) : (
                <fieldset>
                    ...
                    {projects.length === 0 ? (
                        <p className="text-[calc(13px*var(--text-scale,1))] text-faint">
                            No projects exist yet. Create the project in Work
                            first, then come back — you can also share it later
                            from the row above.
                        </p>
                    ) : (
                        <div className="grid gap-1.5 sm:grid-cols-2">
                    ...
                    <p className="mt-1.5 max-w-[70ch] text-[calc(13px*var(--text-scale,1))] text-faint">
                        Leave them all unticked if you&rsquo;re setting the
                        account up ahead of time — an unassigned client sees an
                        empty portal, not somebody else&rsquo;s project.
                    </p>
                </fieldset>
            )}
